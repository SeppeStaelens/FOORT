...
// DECLARATION OF ALL static DiagnosticOptions (for all types of Diagnostics)
// needed here!
std::unique_ptr<GeodesicPositionOptions>
    GeodesicPositionDiagnostic::DiagOptions;
std::unique_ptr<EquatorialPassesOptions>
...
// Sample code:
/*
std::unique_ptr<DiagnosticOptions> MyDiagnostic::DiagOptions; // change
DiagnosticOptions to your descendant struct if necessary
*/
//// END DIAGNOSTIC ADD POINT D.1 ////

// DECLARATION OF ALL static TerminationOptions (for all types of Terminations)
// needed here!
...
std::unique_ptr<ThetaSingularityTermOptions>
    ThetaSingularityTermination::TermOptions;
std::unique_ptr<OriginTermOptions> OriginTermination::TermOptions;
...
///////////////////////////////////////////////////////////////////////////////////
// Config namespace and all of its functions are only defined in
// CONFIGURATION_MODE
#ifdef CONFIGURATION_MODE

// Helper function to look up largecounter
bool Config::lookupValuelargecounter(const ConfigSetting &theSetting,
                                     const char *name, largecounter &value) {
  // the libconfig function lookupValue() only has overloads to return unsigned
  // int and long long (so not unsigned long which is currently used for
  // largecounter!) So instead, we get the setting as a long long and then
  // convert it to a largecounter We need to test various versions of
  // lookupValue because libconfig does not automatically convert integers
  // between smaller/larger types...
  ...
  if (!ret) {
    ...
    // if the long long is too big to fit in a largecounter, then we scale it
    // down to its max value
...
void Config::InitializeScreenOutput(const ConfigObject &theCfg) {
  ...
  if (root.exists("Output")) {
    ...
    // We do these checks to make sure the int we have read in can indeed be
    // interpreted as an OutputLevel
    scroutputint =
        std::max(scroutputint, static_cast<int>(OutputLevel::Level_0_WARNING));
    scroutputint =
        std::min(scroutputint, static_cast<int>(OutputLevel::MaxLevel));
    ...
    lookupValuelargecounter(OutputSettings, "LoopMessageFrequency",
                            loopmessagefrequency);
    SetLoopMessageFrequency(loopmessagefrequency);
  }
}

/// <summary>
/// Config::GetMetric():  Use configuration to create the correct Metric with
...
std::unique_ptr<Metric> Config::GetMetric(const ConfigObject &theCfg) {
  ...
  try {
    ...
    std::transform(
        MetricName.begin(), MetricName.end(), MetricName.begin(),
    ...
    if (MetricName == "kerr") {
      ...
      if (!MetricSettings.lookupValue("a", theKerra)) {
        ScreenOutput("Kerr: no value for a given. Using default: " +
                         std::to_string(theKerra) + ".",
    ...
    } else if (MetricName == "rasheedlarsen" ||
               MetricName == "rasheed-larsen") {
      ...
      if (!MetricSettings.lookupValue("m", RLm)) {
        ScreenOutput("Rasheed-Larsen: no value for m given. Using default: " +
                         std::to_string(RLm) + ".",
                     Output_Other_Default);
      }
      if (!MetricSettings.lookupValue("a", RLa)) {
        ScreenOutput("Rasheed-Larsen: no value for a given. Using default: " +
                         std::to_string(RLa) + ".",
                     Output_Other_Default);
      }
      if (!MetricSettings.lookupValue("p", RLp)) {
        ScreenOutput("Rasheed-Larsen: no value for p given. Using default: " +
                         std::to_string(RLp) + ".",
                     Output_Other_Default);
      }
      if (!MetricSettings.lookupValue("q", RLq)) {
        ScreenOutput("Rasheed-Larsen: no value for q given. Using default: " +
                         std::to_string(RLq) + ".",
                     Output_Other_Default);
      }
      ...
      // All settings complete; create Metric object!
      TheMetric = std::unique_ptr<Metric>(
          new RasheedLarsenMetric(RLm, RLa, RLp, RLq, rLogScale));
      ...
      if (!MetricSettings.lookupValue("a", JOHa)) {
        ScreenOutput("Johannsen: no value for a given. Using default: " +
                         std::to_string(JOHa) + ".",
                     Output_Other_Default);
      }
      if (!MetricSettings.lookupValue("alpha13", JOHa13)) {
        ScreenOutput("Johannsen: no value for alpha13 given. Using default: " +
                         std::to_string(JOHa13) + ".",
                     Output_Other_Default);
      }
      if (!MetricSettings.lookupValue("alpha22", JOHa22)) {
        ScreenOutput("Johannsen: no value for alpha22 given. Using default: " +
                         std::to_string(JOHa22) + ".",
                     Output_Other_Default);
      }
      if (!MetricSettings.lookupValue("alpha52", JOHa52)) {
        ScreenOutput("Johannsen: no value for alpha52 given. Using default: " +
                         std::to_string(JOHa52) + ".",
                     Output_Other_Default);
      }
      if (!MetricSettings.lookupValue("epsilon3", JOHe3)) {
        ScreenOutput("Johannsen: no value for epsilon3 given. Using default: " +
                         std::to_string(JOHe3) + ".",
                     Output_Other_Default);
      }
      ...
      // All settings complete; create Metric object!
      TheMetric = std::unique_ptr<Metric>(
          new JohannsenMetric(JOHa, JOHa13, JOHa22, JOHa52, JOHe3, rLogScale));
      ...
      if (!MetricSettings.lookupValue("a", MNa)) {
        ScreenOutput("Manko-Novikov: no value for a given. Using default: " +
                         std::to_string(MNa) + ".",
                     Output_Other_Default);
      }
      if (!MetricSettings.lookupValue("alpha3", MNalpha3)) {
        ScreenOutput(
            "Manko-Novikov: no value for alpha3 given. Using default: " +
                std::to_string(MNalpha3) + ".",
            Output_Other_Default);
      }
      ...
      // All settings complete; create Metric object!
      TheMetric = std::unique_ptr<Metric>(
          new MankoNovikovMetric(MNa, MNalpha3, rLogScale));
    ...
    else if (MetricName == "mymetric") // remember to use all lower case!
    {
            ...
            // Note: if this option is not present in the configuration file,
    then myparam will not be changed
            // by the call to lookupValue
            MetricSettings.lookupValue("MyParameter", myparam);

            TheMetric = std::unique_ptr<Metric>(new MyMetric{ myparam }); // put
    your additional parameters needed in the constructor
    }
    */
    //// END METRIC ADD POINT B ////
    else // no match found: must be incorrect metric name specified in
         // configuration file
    {
      throw SettingError("No metric settings found.");
    }
  } catch (SettingError &e) {
    // Something happened so that we were unable to determine even which metric
    // to use. Use the default settings (set above at begin of this function)
    ScreenOutput(std::string(e.what()) + " Using default metric (" +
                     DefaultString + ").",
...
/// <summary>
/// Config::GetSource():  Use configuration to create the correct Source with
/// specified parameters
/// </summary>
std::unique_ptr<Source> Config::GetSource(const ConfigObject &theCfg,
  ...
  try {
    ...
    std::transform(
        SourceName.begin(), SourceName.end(), SourceName.begin(),
        [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
    ...
    else // no match found: must be incorrect source name specified in
         // configuration file
    {
      throw SettingError("No source settings found.");
    }
  } catch (SettingError &e) {
    // Something happened so that we were unable to determine even which source
    // to use. Use the default settings (set above at begin of this function)
    // This is not so important since it is just the source --- we can assume
    // this is just NoSource by default
    ScreenOutput(std::string(e.what()) + " Using default source (" +
                     DefaultString + ").",
                 Output_Other_Default);
  }
...
/// <summary>
/// Config::InitializeDiagnostics():  Use configuration to set the Diagnostics
/// bitflag appropriately; initialize all DiagnosticOptions for all Diagnostics
/// that are turned on; and set bitflag for diagnostic to be used for coarseness
/// evaluating in Mesh
/// </summary>
void Config::InitializeDiagnostics(const ConfigObject &theCfg,
  ...
  try {
    ...
    // Helper function: checks to see if a particular Diagnostic (with name
    // Diagname) is present in the Diagnostics options; if the setting "on" is
    // specified; and if "on" is also set to "true" (so by default "on" = false)
    // Only if all these are satisfied do we want to have the Diagnostic
    // included (and initialized)
    auto CheckIfDiagOn = [&AllDiagSettings](const std::string &DiagName) {
      bool diagison{false};
      return AllDiagSettings.exists(
                 DiagName.c_str()) // note that g++ on Fabio's Linux requires
                                   // the .c_str() everywhere
    ...
    if (CheckIfDiagOn("FourColorScreen")) {
      ...
      if (valdiag == Diag_None &&
          AllDiagSettings["FourColorScreen"].lookupValue("UseForMesh", isVal) &&
          isVal) {
    ...
    if (CheckIfDiagOn("GeodesicPosition")) {
      ...
      lookupValuelargecounter(AllDiagSettings["GeodesicPosition"],
                              "UpdateFrequency", updatensteps);
      if (updatensteps == 0) {
        AllDiagSettings["GeodesicPosition"].lookupValue("UpdateStart",
                                                        updatestart);
        AllDiagSettings["GeodesicPosition"].lookupValue("UpdateFinish",
                                                        updatefinish);
      }
      ...
      GeodesicPositionDiagnostic::DiagOptions =
          std::unique_ptr<GeodesicPositionOptions>(new GeodesicPositionOptions{
              outputsteps,
              UpdateFrequency{updatensteps, updatestart, updatefinish}});

      bool isVal{false};
      if (valdiag == Diag_None &&
          AllDiagSettings["GeodesicPosition"].lookupValue("UseForMesh",
                                                          isVal) &&
          isVal) {
        // Use this Diagnostic for the Mesh values
    ...
    if (CheckIfDiagOn("EquatorialPasses")) {
      ...
      bool updatefinish{true};
      lookupValuelargecounter(AllDiagSettings["EquatorialPasses"],
                              "UpdateFrequency", updatensteps);
      if (updatensteps == 0) {
        AllDiagSettings["EquatorialPasses"].lookupValue("UpdateStart",
                                                        updatestart);
        AllDiagSettings["EquatorialPasses"].lookupValue("UpdateFinish",
                                                        updatefinish);
      }
      ...
      EquatorialPassesDiagnostic::DiagOptions =
          std::unique_ptr<EquatorialPassesOptions>(new EquatorialPassesOptions(
              threshold,
              UpdateFrequency{updatensteps, updatestart, updatefinish}));

      bool isVal{false};
      if (valdiag == Diag_None &&
          AllDiagSettings["EquatorialPasses"].lookupValue("UseForMesh",
                                                          isVal) &&
          isVal) {
        // Use this Diagnostic for the Mesh values
    ...
    if (CheckIfDiagOn("ClosestRadius")) {
      ...
      bool updatestart{true};
      bool updatefinish{true};
      lookupValuelargecounter(AllDiagSettings["ClosestRadius"],
                              "UpdateFrequency", updatensteps);
      if (updatensteps == 0) {
        AllDiagSettings["ClosestRadius"].lookupValue("UpdateStart",
                                                     updatestart);
        AllDiagSettings["ClosestRadius"].lookupValue("UpdateFinish",
                                                     updatefinish);
      }

      // Check if metric is a spherical horizon metric with integration set to a
      // logarithmic r coordinate, if so, pass this along to the options struct
      bool rlogradius{false};
      const SphericalHorizonMetric *sphermetric =
      ...
      ClosestRadiusDiagnostic::DiagOptions =
          std::unique_ptr<ClosestRadiusOptions>(new ClosestRadiusOptions(
              rlogradius,
              UpdateFrequency{updatensteps, updatestart, updatefinish}));

      bool isVal{false};
      if (valdiag == Diag_None &&
          AllDiagSettings["ClosestRadius"].lookupValue("UseForMesh", isVal) &&
          isVal) {
        // Use this Diagnostic for the Mesh values
    ...
    // Add a check to see if your new Diagnostic is turned on here. If it is,
    // check any further options it needs and make sure to set the diagnostic
    // flags appropriately. Sample code:
    /*
    // MyDiagnostic
    if (CheckIfDiagOn("MyDiagnostic"))
    {
            alldiags |= Diag_MyDiagnostic; // use the flag you created at
    DIAGNOSTIC ADD POINT B.

            // The following assumes your new Diagnostic carries a static
    DiagnosticOptions struct.
            // If it does not, or if it carries additional options (in a
    descendant struct of DiagnosticOptions),
            // then update this accordingly
            largecounter updatensteps = 1;
            bool updatestart{ true };
            bool updatefinish{ true };
            lookupValuelargecounter(AllDiagSettings["MyDiagnostic"],
    "UpdateFrequency", updatensteps); if (updatensteps == 0)
            {
                    AllDiagSettings["MyDiagnostic"].lookupValue("UpdateStart",
    updatestart); AllDiagSettings["MyDiagnostic"].lookupValue("UpdateFinish",
    updatefinish);
            }
            // (look up any additional options here...)

            MyDiagnostic::DiagOptions =
                    std::unique_ptr<DiagnosticOptions>(new DiagnosticOptions{
    UpdateFrequency{updatensteps, updatestart,updatefinish} });

            bool isVal{ false };
            if (valdiag == Diag_None &&
    AllDiagSettings["MyDiagnostic"].lookupValue("UseForMesh", isVal) && isVal)
            {
                    // Use this Diagnostic for the Mesh values
                    valdiag = Diag_MyDiagnostic; // use the flag you created at
    DIAGNOSTIC ADD POINT B.
            }
    }
    */
    ...
    // Make sure something has been specified as the value Diagnostic (for the
    // Mesh) If not, then set it to the FourColorScreen Diagnostic by default if
    // FourColorScreen is present If also FourColorScreen is not turned on, then
    // revert to all defaults for Diagnostics.
    if (valdiag == Diag_None) {
      if (alldiags & Diag_FourColorScreen) {
        ScreenOutput("No mesh diagnostic set; using FourColorScreen.",
                     Output_Other_Default);
        valdiag = Diag_FourColorScreen;
      } else {
        throw SettingError("Diagnostics turned on but no Mesh diagnostic "
                           "selected, and FourColorScreen not turned on.");
      }
    }
  } catch (SettingError &e) {
    // Something happened (e.g. no Diagnostic options specified) that makes us
    // revert completely to default Diagnostic options (which are SET HERE)
    ScreenOutput(std::string(e.what()) + " Using default diagnostic(s) (" +
                     "FourColorScreen" + ").",
                 Output_Important_Default);
    alldiags = Diag_FourColorScreen;
    valdiag = Diag_FourColorScreen;
...
/// <summary>
/// Config::InitializeTerminations():  Use configuration to set the Termination
/// bitflag appropriately; and initialize all TerminationOptions for all
/// Terminations that are turned on.
/// </summary>
void Config::InitializeTerminations(const ConfigObject &theCfg,
  ...
  try {
    ...
    // Helper function: checks to see if a particular Termination (with name
    // TermName) is present in the Terminations options; if the setting "on" is
    // specified; and if "on" is also set to "true" (so by default "on" = false)
    // Only if all these are satisfied do we want to have the Termination
    // included (and initialized)
    auto CheckIfTermOn = [&AllTermSettings](const std::string &TermName) {
      bool termison{false};
      return AllTermSettings.exists(
                 TermName.c_str()) // note that g++ on Fabio's Linux requires
                                   // the .c_str() everywhere
    ...
    if (CheckIfTermOn("Horizon")) {
      ...
      const SphericalHorizonMetric *sphermetric =
          dynamic_cast<const SphericalHorizonMetric *>(theMetric);
      if (!sphermetric) {
        ScreenOutput("Horizon Termination turned on but metric does not have "
                     "horizon! Turning off Horizon Termination.",
                     Output_Important_Default);
      } else {
        // Horizon
        ...
        // Setting to look up: (relative) radial distance to horizon before
        ...
        lookupValuelargecounter(AllTermSettings["Horizon"], "UpdateFrequency",
                                updatefreq);
        ...
        HorizonTermination::TermOptions =
            std::unique_ptr<HorizonTermOptions>(new HorizonTermOptions{
                horizonRadius, rLogScale, epsHorizon, updatefreq});
      }
    }
    ...
    if (CheckIfTermOn("BoundarySphere")) {
      ...
      const SphericalHorizonMetric *sphermetric =
          dynamic_cast<const SphericalHorizonMetric *>(theMetric);
      const BosonStarMetric *bsmetric =
          dynamic_cast<const BosonStarMetric *>(theMetric);
      if (sphermetric && sphermetric->getrLogScale() ||
          bsmetric && bsmetric->getrLogScale())
        rlogradius = true;

      // By default, this Termination updates every step
      // Check to see if a different update frequency has been specified
      largecounter updatefreq = 1;
      lookupValuelargecounter(AllTermSettings["BoundarySphere"],
                              "UpdateFrequency", updatefreq);

      // Initialize the (static) TerminationOptions for BoundarySphere!
      BoundarySphereTermination::TermOptions =
          std::unique_ptr<BoundarySphereTermOptions>(
              new BoundarySphereTermOptions{radius, rlogradius, updatefreq});
    }
    ...
    if (CheckIfTermOn("TimeOut")) {
      ...
      lookupValuelargecounter(AllTermSettings["TimeOut"], "MaxSteps",
                              timeoutsteps);

      // By default, this Termination updates every step
      // Check to see if a different update frequency has been specified
      largecounter updatefreq = 1;
      lookupValuelargecounter(AllTermSettings["TimeOut"], "UpdateFrequency",
                              updatefreq);

      // Initialize the (static) TerminationOptions for TimeOut!
      TimeOutTermination::TermOptions = std::unique_ptr<TimeOutTermOptions>(
          new TimeOutTermOptions{timeoutsteps, updatefreq});
    }
    ...
    if (CheckIfTermOn("ThetaSingularity")) {
      ...
      // get the tolerance for how much theta is allowed to get near the poles;
      // default 1e-5
      real epsilon{1e-5};
      ...
      // Check to see if a different update frequency has been specified
      largecounter updatefreq = 1;
      lookupValuelargecounter(AllTermSettings["ThetaSingularity"],
                              "UpdateFrequency", updatefreq);

      // Initialize the (static) TerminationOptions for ThetaSingularity!
      ThetaSingularityTermination::TermOptions =
          std::unique_ptr<ThetaSingularityTermOptions>(
              new ThetaSingularityTermOptions{epsilon, updatefreq});
    }

    // Origin
    if (CheckIfTermOn("Origin")) {
      // Origin termination on! Add it to bitflag.
      allterms |= Term_OriginSingularity;

      // Setting to look up: (relative) radial distance to origin before
      // stopping integration
      real epsOrigin{0.001};
      AllTermSettings["Origin"].lookupValue("Epsilon_Origin", epsOrigin);

      // Check if metric is a boson star metric with integration set to a
      // logarithmic r coordinate, if so, pass this along to the options struct
      bool rlogradius{false};
      const BosonStarMetric *bsmetric =
          dynamic_cast<const BosonStarMetric *>(theMetric);
      if (bsmetric && bsmetric->getrLogScale())
        rlogradius = true;

      // By default, this Termination updates every step
      // Check to see if a different update frequency has been specified
      largecounter updatefreq = 1;
      lookupValuelargecounter(AllTermSettings["Origin"], "UpdateFrequency",
                              updatefreq);

      // Initialize the (static) TerminationOptions for BoundarySphere!
      OriginTermination::TermOptions = std::unique_ptr<OriginTermOptions>(
          new OriginTermOptions{epsOrigin, rlogradius, updatefreq});
    }

    //// TERMINATION ADD POINT D.2. ////
    // Check to see if your new Termination has been turned on, and if so, add
    // it to the allterms bitflag and set its options accordingly. Sample code:
    /*
    // MyTermination
    if (CheckIfTermOn("MyTermination"))
    {
            allterms |= Term_MyTermination; // use the bitflag you defined at
    TERMINATION ADD POINT B.1.

            // The following assumes that MyTermination has a static
    TerminationOptions struct,
            // if it does not or has additional options (i.e. it has a
    descendant of TerminationOptions as its
            // static struct), then update here accordingly.
            largecounter updatefreq = 1;
            lookupValuelargecounter(AllTermSettings["MyTermination"],
    "UpdateFrequency", updatefreq);

            // Initialize the (static) TerminationOptions!
            MyTermination::TermOptions =
                    std::unique_ptr<TerminationOptions>(new TerminationOptions{
    updatefreq });
    }
    */
    ...
    // Something happened (e.g. no Termination options specified) that makes us
    // revert completely to default Termination options (which are SET HERE)
    ScreenOutput(std::string(e.what()) + " Using default termination(s) (" +
                     "BoundarySphere and TimeOut" + ").",
                 Output_Important_Default);
    allterms = Term_BoundarySphere | Term_TimeOut;
    BoundarySphereTermination::TermOptions =
        std::unique_ptr<BoundarySphereTermOptions>(
            new BoundarySphereTermOptions{
                1000,
                dynamic_cast<const SphericalHorizonMetric *>(theMetric) &&
                    (dynamic_cast<const SphericalHorizonMetric *>(theMetric))
                        ->getrLogScale(),
...
/// </summary>
std::unique_ptr<ViewScreen>
Config::GetViewScreen(const ConfigObject &theCfg, DiagBitflag valdiag,
                      const Metric *const theMetric) {
  ConfigSetting &root = theCfg.getRoot();
  ...
  std::unique_ptr<ViewScreen> theViewScreen{new ViewScreen(
      pos, dir, screensize, screencenter, std::move(theMesh), theMetric)};
...
/// </summary>
std::unique_ptr<Mesh> Config::GetMesh(const ConfigObject &theCfg,
                                      DiagBitflag valdiag) {
  // Get the root configuration settings
  ConfigSetting &root = theCfg.getRoot();
  ...
  try {
    ...
    // We have read in the Mesh type. Now we select and create a Mesh
    // accordingly
    if (meshname == "SimpleSquareMesh") {
      ...
      theMesh =
          std::unique_ptr<Mesh>(new SimpleSquareMesh(totalpixels, valdiag));
    } else if (meshname == "InputCertainPixelsMesh") {
      // This Mesh will ask the user to input pixels on the grid specified in
      // the configuration file. (the Mesh will ask for input in its
      // constructor, so here!)
      largecounter totalpixels{100 * 100};
      lookupValuelargecounter(MeshSettings, "TotalPixels", totalpixels);

      theMesh = std::unique_ptr<Mesh>(
          new InputCertainPixelsMesh(totalpixels, valdiag));
      ...
      {
        ScreenOutput(
            "Invalid MaxSubdivide level given. Using MaxSubdivide = 1.",
            Output_Other_Default);
        maxsubdivide = 1;
      }
      MeshSettings.lookupValue("InitialSubdivisionToFinal", initialsubtofinal);

      theMesh = std::unique_ptr<Mesh>(new SquareSubdivisionMesh(
          maxpixels, initialpixels, maxsubdivide, iterationpixels,
          initialsubtofinal, valdiag));
      ...
      theMesh = std::unique_ptr<Mesh>(new SquareSubdivisionMeshV2(
          maxpixels, initialpixels, maxsubdivide, iterationpixels,
          initialsubtofinal, valdiag));
    }
    ...
    ScreenOutput(std::string(e.what()) +
                     " Using default (SimpleSquareMesh with 100x100 pixels).",
                 Output_Important_Default);
...
/// <summary>
/// Config::GetGeodesicIntegrator():  Returns a pointer to the integrator
/// function to be used as specified in the configuration file.
/// </summary>
GeodesicIntegratorFunc
Config::GetGeodesicIntegrator(const ConfigObject &theCfg) {
  ...
  try {
    ...
    if (!IntegratorSettings.lookupValue("StepSize", stepsize)) {
      ScreenOutput("Using default integrator stepsize: " +
                       std::to_string(Integrators::epsilon) + ".",
                   Output_Other_Default);
    } else {
    ...
    std::transform(
        IntegratorType.begin(), IntegratorType.end(), IntegratorType.begin(),
        [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
    ...
    if (IntegratorType == "rk4") {
      ...
      // Get the velocity tolerance (fractional difference in intermediate and
      // final velocities for a step)
      real verlettolerance{Integrators::VerletVelocityTolerance};
      IntegratorSettings.lookupValue("VerletVelocityTolerance",
                                     verlettolerance);
      Integrators::VerletVelocityTolerance = verlettolerance;
    }
    // else if ... (other integrators here)
    else // no match found: must be incorrect integrator type specified in
         // configuration file
    {
      throw SettingError("No integrator settings found.");
    }
  } catch (SettingError &e) {
    // Something happened so that we were unable to determine even which
    // integrator to use. Use the default settings (set above at begin of this
    // function)
    ScreenOutput(std::string(e.what()) + " Using default integrator (" +
                     DefaultString + ").",
                 Output_Important_Default);
  }
...
/// <summary>
/// Config::GetOutputHandler():  Creates the GeodesicOutputHandler object with
/// options specified according to the configuration file, for handling of
/// geodesic outputs.
/// </summary>
std::unique_ptr<GeodesicOutputHandler>
Config::GetOutputHandler(const ConfigObject &theCfg, DiagBitflag alldiags,
                         DiagBitflag valdiag, std::string FirstLineInfo) {
  // First populate a helper vector of strings of the names of all diagnostics
  std::vector<std::string> diagstrings{
      std::move(Utilities::GetDiagNameStrings(alldiags, valdiag))};

  // DEFAULTS
  std::unique_ptr<GeodesicOutputHandler> TheHandler{
      new GeodesicOutputHandler("", "", "", diagstrings)};
  ...
  try {
    ...
    largecounter nrToCache{LARGECOUNTER_MAX -
                           1}; // default is essentially infinite
    lookupValuelargecounter(OutputSettings, "GeodesicsToCache", nrToCache);

    // Max number of geodesics to write to file
    largecounter GeodesicsPerFile{
        LARGECOUNTER_MAX}; // default is essentially infinite
    lookupValuelargecounter(OutputSettings, "GeodesicsPerFile",
                            GeodesicsPerFile);
    ...
    TheHandler =
        std::unique_ptr<GeodesicOutputHandler>(new GeodesicOutputHandler(
            FilePrefix, TimeStampStr, FileExtension, diagstrings, nrToCache,
