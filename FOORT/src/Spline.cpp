...
tk::spline::spline() : m_type(cspline),
                       m_left(second_deriv), m_right(second_deriv),
                       m_left_value(0.0), m_right_value(0.0), m_made_monotonic(false)
{
    ;
}
tk::spline::spline(const std::vector<double> &X, const std::vector<double> &Y,
                   spline_type type, bool make_monotonic, bd_type left,
                   double left_value, bd_type right, double right_value) : m_type(type), m_left(left), m_right(right),
...
std::vector<double> tk::spline::get_x() const { return m_x; }
std::vector<double> tk::spline::get_y() const { return m_y; }
...
tk::internal::band_matrix::band_matrix(int dim, int n_u, int n_l)
{
    resize(dim, n_u, n_l);
}
void tk::internal::band_matrix::resize(int dim, int n_u, int n_l)
{
    assert(dim > 0);
    assert(n_u >= 0);
    assert(n_l >= 0);
...
double &tk::internal::band_matrix::operator()(int i, int j)
{
...
double tk::internal::band_matrix::operator()(int i, int j) const
{
    int k = j - i; // what band is the entry
    assert((i >= 0) && (i < dim()) && (j >= 0) && (j < dim()));
    assert((-num_lower() <= k) && (k <= num_upper()));
