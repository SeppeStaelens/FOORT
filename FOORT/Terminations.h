...
// Forward declaration of Geodesic class needed here, since Diagnostics are
// passed a pointer to their owner Geodesic (note "Geodesic.h" is NOT included
// to avoid header loop, and we do not need Geodesic member functions here!)
class Geodesic;
...
constexpr TermBitflag Term_OriginSingularity{0b0000'0000'0001'0000};

//// TERMINATION ADD POINT B1 ////
// Add a TermBitflag for your new Termination. Make sure you use a bitflag that
// has not been used before! Sample code:
/*
constexpr TermBitflag Term_MyTerm				{
...
enum class Term {
  Uninitialized = -1, // Geodesic has not been properly initialized yet with
                      // initial position/velocity
  Continue = 0,       // All is right, continue integrating geodesic
  Horizon,            // STOP, encountered horizon (set by HorizonTermination)
  BoundarySphere,     // STOP, encountered boundary sphere (set by
                      // BoundarySphereTermination)
  TimeOut,            // STOP, taken too many steps (set by TimeOutTermination)
  ThetaSingularity,  // STOP, too close to polar coordinate singularity (theta =
                     // 0 or theta = pi/2)
  OriginSingularity, // STOP, too close to the origin (r = 0)
...
public:
  ...
  // Resets Termination object. This is called when the owner Geodesic is reset
  // in order to start integrating a new geodesic. The base class implementation
  // only resets m_StepsSinceUpdated Descendants can override this if they need
  // to reset additional internal variables
  virtual void Reset();
...
protected:
  ...
  // Helper function to decide if the Termination should indeed update its
  // status, based on UpdateNSteps (which is set to 0 if we always update)
  bool DecideUpdate(largecounter UpdateNSteps);

  // The termination is itself in charge of keeping track of how many steps it
  // has been since it has been updated The Termination's TerminationOptions
  // struct tells it how many steps it needs to wait between updates
...
public:
  ...
  // Options (contains horizon radius, if we are using logarithmic r coordinate,
  // and distance allowed from the horizon)
  static std::unique_ptr<HorizonTermOptions> TermOptions;
};

// Forward declaration needed before Termination
struct BoundarySphereTermOptions;
// The Boundary Sphere: this terminates the geodesic (and returns
// Term::BoundarySphere) if the geodesic reaches outside of the boundary sphere
class BoundarySphereTermination final : public Termination {
public:
  // Basic constructor only passes on Geodesic pointer to base class constructor
  BoundarySphereTermination(Geodesic *const theGeodesic)
      : Termination(theGeodesic) {}
  ...
  // The options that the BoundarySphereTermination keeps (contains the radius
...
public:
  ...
  // The options that the TimeOutTermination keeps (contains max number of steps
  // allowed)
...
public:
  ...
  // The options that the Termination keeps (will probably be a descendant
  // struct instead, which specifies any additional options the Termination
  // needs)
  static std::unique_ptr<ThetaSingularityTermOptions> TermOptions;
};

// Forward declaration needed before Termination
struct OriginTermOptions;
// Origin termination: terminate geodesics if they get too close to the origin
// (returns Term::Origin)
class OriginTermination final : public Termination {
public:
  // Basic constructor only passes on Geodesic pointer to base class constructor
  OriginTermination(Geodesic *const theGeodesic) : Termination(theGeodesic) {}

  // Check if we are too close to the origin
  Term CheckTermination() final;

  // Description string
  std::string getFullDescriptionStr() const final;

  // Options (contains whether we are using logarithmic r coordinate, and
  // distance allowed from the origin)
  static std::unique_ptr<OriginTermOptions> TermOptions;
};
...
/*
// Forward declaration needed before Termination
struct TerminationOptions; // possibly will instead need to declare descendant
options struct class MyTermination final : public Termination // good practice
to make the class final unless descendant classes are possible
{
public:
        // Constructor must at least pass on Geodesic pointer to base class
constructor MyTermination(Geodesic* const theGeodesic) :
Termination(theGeodesic) {}

        // Do you need to reset any internal variables specific to
MyTermination? If so, override Reset() (This is not mandatory)
        // Note: make sure to call the base class implementation
Termination::Reset()
        // from within your implementation of MyTermination::Reset(), so that
the base class internal variable is also reset! void Reset() final;
        ...
        // The options that the Termination keeps (will probably be a descendant
struct instead, which specifies
...
// Base class for TerminationOptions. Other Terminations can inherit from here
// if they require more options.
...
// Options class for HorizonTermination; keeps track of location of horizon
// radius and the epsilon to terminate away from the horizon
struct HorizonTermOptions : public TerminationOptions {
public:
  HorizonTermOptions(real theHorizonRadius, bool therLogScale,
                     real theAtHorizonEps, largecounter Nsteps)
      : HorizonRadius{theHorizonRadius}, AtHorizonEps{theAtHorizonEps},
        rLogScale{therLogScale}, TerminationOptions(Nsteps) {}
...
// Options class for BoundarySphere; has to keep track of the BoundarySphere's
// radius
struct BoundarySphereTermOptions : public TerminationOptions {
public:
  BoundarySphereTermOptions(real theRadius, bool therLogScale,
                            largecounter Nsteps)
...
// Options class for TimeOut; has to keep track of the max. number of
// integration steps allowed
struct TimeOutTermOptions : public TerminationOptions {
public:
  TimeOutTermOptions(largecounter MaxStepsAllowed, largecounter Nsteps)
      : MaxSteps{MaxStepsAllowed}, TerminationOptions(Nsteps) {}
...
public:
  ThetaSingularityTermOptions(real epsilon, largecounter Nsteps)
      : ThetaSingEpsilon{epsilon}, TerminationOptions(Nsteps) {}
...
// Options class for BoundarySphere; has to keep track of the BoundarySphere's
// radius
struct OriginTermOptions : public TerminationOptions {
public:
  OriginTermOptions(real theOriginEps, bool therLogScale, largecounter Nsteps)
      : OriginEps{theOriginEps}, rLogScale{therLogScale},
        TerminationOptions(Nsteps) {}

  const real OriginEps;
  const bool rLogScale;
};

//// TERMINATION ADD POINT A2 ////
// Add your new TerminationOptions struct here, inheriting from
// TerminationOptions (if needed) Sample code:
/*
struct MyTermOptions : public TerminationOptions
{
public:
        MyTermOptions(..., largecounter Nsteps) : TerminationOptions(Nsteps) //,
