...
TEST(Integrators, rk4_bound_geodesic)
{
    ...
    TimeOutTermination::TermOptions =
        std::unique_ptr<TimeOutTermOptions>(new TimeOutTermOptions{500, 1});
    ...
    Point initpos = {0.0, 3.0 - 1.e-10, pi / 2.0, 0.0};
    ...
    EXPECT_NEAR(final_r, 3.0, 0.001) << "Final radial position too large after 5 half-orbits: " << final_r << std::endl;
...
TEST(Integrators, verlet_bound_geodesic)
{
    ...
    // Set up initial conditions for the bound geodesic
    Point initpos = {0.0, 3.0 + 1e-16, pi / 2.0, 0.0};
    ...
    while (theGeod.getTermCondition() == Term::Continue && current_phi < 5 * pi)
    {
        ...
        current_pos = theGeod.getCurrentPos();
        current_phi = current_pos[3];
        std::cout << "Current radius: " << current_pos[1] << std::endl;
    }
