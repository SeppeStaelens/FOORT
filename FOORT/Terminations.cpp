...
TerminationUniqueVector CreateTerminationVector(TermBitflag termflags,
  ...
  if (termflags & Term_OriginSingularity) {
    theTermVector.emplace_back(new OriginTermination{theGeodesic});
  }
  //// TERMINATION ADD POINT C ////
  // Add an if statement that checks if your Termination's TermBitflag is turned
  // on, if so add a new instance of it to theTermVector. Sample code:
  /*
  // Is MyTerm turned on?
...
// This helper function returns true if the Termination should update its
// internal status. Should be called from within CheckTermination() with the
// appropriate TermOptions::UpdateEveryNSteps
...
Term HorizonTermination::CheckTermination() {
  ...
  if (DecideUpdate(TermOptions->UpdateEveryNSteps)) {
    ...
    // if we are using a logarithmic r scale (u=log(r)) then first exponentiate
    // to get true radius
    real thegeodesicr = (m_OwnerGeodesic->getCurrentPos())[1];
    real r = TermOptions->rLogScale ? exp(thegeodesicr) : thegeodesicr;
    // Check if we are almost at the horizon; the second check is for horizons
    // which are at r=0
...
std::string HorizonTermination::getFullDescriptionStr() const {
  ...
  return "Horizon (stop at " + std::to_string(1 + TermOptions->AtHorizonEps) +
         "x(horizon radius))";
}
...
// Check to see if enough steps have been taken to time out, if so return
// Term::TimeOut
Term TimeOutTermination::CheckTermination() {
  Term ret = Term::Continue;

  if (DecideUpdate(TermOptions->UpdateEveryNSteps)) {
    // Check to see if we have done enough steps to time out the integration
    // already
...
std::string TimeOutTermination::getFullDescriptionStr() const {
  // Full description string
  return "Time out (max integration steps: " +
         std::to_string(TermOptions->MaxSteps) + ")";
}
...
std::string ThetaSingularityTermination::getFullDescriptionStr() const {
  // Full description string
  return "Theta singularity (epsilon: " +
         std::to_string(TermOptions->ThetaSingEpsilon) + ")";
}

/// <summary>
/// OriginTermination functions
/// </summary>

// Check to see if the origin is reached, if so return Term::BoundarySphere
Term OriginTermination::CheckTermination() {
  Term ret = Term::Continue;

  // Check if we are allowed to update (i.e. check for termination)
  if (DecideUpdate(TermOptions->UpdateEveryNSteps)) {
    // Check to see if we are almost at the origin
    // if we are using a logarithmic r scale (u=log(r)) then first exponentiate
    // to get true radius
    real thegeodesicr = (m_OwnerGeodesic->getCurrentPos())[1];
    real r = TermOptions->rLogScale ? exp(thegeodesicr) : thegeodesicr;
    // Check if we are almost at the origin; the second check is for horizons
    // which are at r=0
    if (r < TermOptions->OriginEps) {
      ret = Term::Horizon;
      std::cout << "Term origin" << r << " and " << TermOptions->OriginEps;
    }
  }
...
std::string OriginTermination::getFullDescriptionStr() const {
  // Full description string
  return "Origin (stop at " + std::to_string(TermOptions->OriginEps) + ")";
}
